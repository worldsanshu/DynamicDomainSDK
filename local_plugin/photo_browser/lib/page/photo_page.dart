...
class _PhotoPageState extends State<PhotoPage> with TickerProviderStateMixin, PageMixin {
  ...
  Widget build(BuildContext context) {
    if (_imageProviderInfo.status == ImageLoadStatus.failed &&
        (_thumbImageProvideInfo == null || _thumbImageProvideInfo?.status == ImageLoadStatus.failed)) {
    ...
    return LayoutBuilder(
      builder: (
        ...
        Widget content;
        if (_thumbImageProvideInfo == null || _imageProviderInfo.status == ImageLoadStatus.completed) {
          content = _buildContent(context, constraints, _imageProviderInfo);
        } else {
          content = Stack(children: [
            Positioned.fill(child: _buildContent(context, constraints, _thumbImageProvideInfo!)),
            const Positioned.fill(child: CupertinoActivityIndicator(color: Colors.blue)),
          ]);
        }

        Color backColor = widget.backcolor == Colors.transparent ? Colors.transparent : widget.backcolor.withOpacity(mPullDownBgColorScale);
  ...
  Widget _buildContent(BuildContext context, BoxConstraints constraints, ImageProviderInfo providerInfo) {
    return Stack(
      children: [
        Positioned.fill(child: _buildImage(constraints, providerInfo.imageProvider)),
  ...
  Widget _buildImage(BoxConstraints constraints, ImageProvider imageProvider) {
    return Container(
      ...
      child: widget.heroTag != null ? _buildHeroImage(imageProvider) : _buildTransformImage(imageProvider),
    );
  }
  ...
  Widget _buildHeroImage(ImageProvider imageProvider) {
    if (widget.willPop) {
      ...
      double y = mOffset.dy + (mConstraints!.maxHeight - mImageDefH) * mScale * 0.5;
    ...
    return Hero(
      ...
      flightShuttleBuilder: (context, animation, direction, fromContext, toContext) {
        ...
        return AnimatedContainer(
          ...
          child: direction == HeroFlightDirection.pop ? fromHero : toHero,
        );
      },
  ...
  Image _createImage({required ImageProvider image, double? width, double? height}) {
    return Image(
      image: image,
  ...
  Widget _buildLoading({ImageChunkEvent? imageChunkEvent}) {
    double progress = 0.0;
    if (imageChunkEvent?.cumulativeBytesLoaded != null && imageChunkEvent?.expectedTotalBytes != null) {
