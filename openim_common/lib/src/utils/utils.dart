...
class IMUtils {
  ...
  static Future<CroppedFile?> uCrop(String path) {
    return ImageCropper().cropImage(
      sourcePath: path,
      // aspectRatioPresets: [ // Removed in new version
    ...
    if (Platform.isAndroid) {
      try {
        externalStorageDirPath =
            '/storage/emulated/0/Download'; //await AndroidPathProvider.downloadsPath;
      } catch (err, st) {
  ...
  static List<Message> calChatTimeInterval(List<Message> list,
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (index <= list.length - 1) {
        ...
        if (next.difference(cur).inMinutes > 2) {
  ...
  static String getChatTimeline(int ms, [String formatToday = 'HH:mm']) {
    ...
    if (isSameWeek(dateTime, now)) {
      ...
      final weekDayStr = isChinese ? weekDayChinese[weekDay]! : weekDay;
      final timeStr = formatter.format(dateTime);
      return '$weekDayStr $timeStr';
    }

    if (dateTime.year == now.year) {
      final dateFormat = isChinese ? 'MM月dd日' : 'MM/dd';
      final dateStr = DateFormat(dateFormat).format(dateTime);
      final timeStr = formatter.format(dateTime);
      return '$dateStr $timeStr';
    }

    final dateFormat = isChinese ? 'yyyy年MM月dd日' : 'yyyy/MM/dd';
    final dateStr = DateFormat(dateFormat).format(dateTime);
    final timeStr = formatter.format(dateTime);
    return '$dateStr $timeStr';
  }
  ...
  static bool isSameWeek(DateTime date1, DateTime date2) {
    ...
    return date1.isAfter(weekStart.subtract(const Duration(days: 1))) &&
        date1.isBefore(weekEnd.add(const Duration(days: 1)));
  }
    ...
    navigator!.push(PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) {
          return ChatVideoPlayerView(
            ...
            heroTag: null,
            onDownload: (url, file) {
              // Use existing Permissions helper
              Permissions.photos(() {
                if (file != null) {
      ...
      if (isAvailableFileSize) {
        ...
        if (isExitNetwork) {
          if (isExitCachePath) {
            ...
            final cacheFileMimeType = lookupMimeType(cachePath);
            if (ext != null && cacheFileMimeType?.contains(ext) == true) {
            } else {
              cachePath = '$dir/${name}_${message.clientMsgID}.$ext';
            }
          }
  ...
  static Future previewMediaFile(
    ...
    /// Kiểm tra và yêu cầu quyền lưu ảnh/video
    ///
    /// Hàm này sẽ:
    /// 1. Kiểm tra quyền hiện tại
    /// 2. Nếu chưa có quyền, hiển thị popup yêu cầu quyền
    /// 3. Trả về true nếu được cấp quyền, false nếu bị từ chối
    ///
    /// Sử dụng Permissions.photos để xử lý quyền lưu ảnh/video
    /// trên cả Android và iOS
    /// Tải và lưu video từ URL
    Future<void> _downloadAndSaveVideo(String url) async {
      try {
        EasyLoading.showProgress(0, status: '0%');

        final downloader =
            MultiThreadDownloader(url: url, fileName: url.split('/').last);
        ...
        if (filePath != null) {
          await HttpUtil.saveFileToGallerySaver(File(filePath),
              showToast: LoadingView.singleton.isProgressVisible);
        } else {
          IMViews.showToast('Failed to download video');
        }
      } catch (e) {
        EasyLoading.dismiss();
        Logger.print('Error downloading video: $e');
        IMViews.showToast('Error downloading video: $e');
      }
    }

    void _saveMediaInternal(BuildContext ctx, String url) async {
      ...
      if (cached != null) {
        // Sử dụng hàm helper để tìm file video thực tế từ cache
        final actualVideoFile =
            await IMUtils.findActualVideoFileFromCache(cached, url);

        if (actualVideoFile != null) {
          LoadingView.singleton.show();
          try {
            await HttpUtil.saveFileToGallerySaver(actualVideoFile);
            LoadingView.singleton.dismiss();
            return;
          } catch (e) {
            Logger.print('Failed to save actual video file from cache: $e');
            LoadingView.singleton.dismiss();
            // Nếu lưu file cache thất bại, tiếp tục tải từ URL
          }
        } else {
          Logger.print('No actual video file found in cache: ${cached.path}');
        }
      }

      // Tải video từ URL gốc nếu không có cache hợp lệ
      await _downloadAndSaveVideo(url);
    }

    /// Lưu ảnh với kiểm tra quyền
    Future<void> _saveImageWithPermission(String url) async {
      Permissions.photos(() async {
        ...
        try {
          ...
          LoadingView.singleton.dismiss();
          Logger.print('Error saving image: $e');
          IMViews.showToast('Failed to save image');
        }
      });
    }

    /// Lưu video với kiểm tra quyền
    Future<void> _saveVideoWithPermission(String url) async {
      Permissions.photos(() async {
        _saveMediaInternal(context, url);
      });
    }

    /// Lưu media nội bộ (không kiểm tra quyền)

    /// Lưu media (video/ảnh) về máy
    ///
    /// Xử lý vấn đề cache video:
    /// - CachedVideoControllerService thường trả về file .bin không thể lưu trực tiếp
    /// - Sử dụng findActualVideoFileFromCache để tìm file video thực tế
    /// - Nếu không tìm thấy, tải lại từ URL gốc
    void saveMedia(BuildContext ctx, String url) async {
      try {
        final cachedVideoControllerService =
            CachedVideoControllerService(DefaultCacheManager());
        final cached = await cachedVideoControllerService.getCacheFile(url);
        ...
        if (cached != null) {
          LoadingView.singleton.show();
          await HttpUtil.saveFileToGallerySaver(cached);
          LoadingView.singleton.dismiss();
        } else {
          EasyLoading.showProgress(0, status: '0%');
        ...
        EasyLoading.dismiss();
        Logger.print('Save media error: $e');
        IMViews.showToast(StrRes.saveFailed);
      }
    }
          ...
          onPressedButton: (type) async {
            ...
            switch (type) {
              case OperateType.save:
                if (msg.isVideoType) {
                  _saveVideoWithPermission(msg.videoElem!.videoUrl!);
                } else {
                  final url = msg.pictureElem?.sourcePicture?.url;
                  if (url?.isNotEmpty == true) {
                    // Lưu ảnh với kiểm tra quyền
                    _saveImageWithPermission(url!);
                  }
                }
                break;
    ...
    Positioned buildSaveImageBtn(
      ...
      return Positioned(
        ...
        child: GestureDetector(
            onTap: () async {
              if (msg.isVideoType) {
                await _saveVideoWithPermission(msg.videoElem!.videoUrl!);
              } else {
                final url = msg.pictureElem?.sourcePicture?.url;
                if (url?.isNotEmpty == true) {
                  // Lưu ảnh với kiểm tra quyền
                  await _saveImageWithPermission(url!);
                }
              }
            },
            child: const Icon(
    ...
    try {
      photoBrowser = PhotoBrowser(
        ...
        customChildBuilder: (int index) {
          ...
          final snapshotUrl =
              msg.videoElem?.snapshotUrl?.adjustThumbnailAbsoluteString(960) ??
                  '';
          final rawUrl = msg.videoElem?.videoUrl;
          final String? url = IMUtils.isValidUrl(rawUrl) ? rawUrl : null;
          final String? videoPath = msg.videoElem?.videoPath;
          ...
          return CustomChild(
            child: Center(
              ...
              muted: muted,
              onDownload: (u, file) async {
                if (u != null) {
                  _saveVideoWithPermission(u);
                }
              },
            )),
            allowZoom: true,
        ...
        positions: (BuildContext context) =>
            <Positioned>[buildCloseBtn(context)],
      ...
      return photoBrowser.push(context, fullscreenDialog: false);
    } catch (e) {
      IMViews.showToast(e.toString());
  ...
  static openFileByOtherApp(String path) async {
    ...
    if (result.type == ResultType.noAppToOpen) {
      IMViews.showToast(StrRes.noSupportedApp);
    } else if (result.type == ResultType.permissionDenied) {
      IMViews.showToast(StrRes.noPermissionAccess);
    } else if (result.type == ResultType.fileNotFound) {
      IMViews.showToast(StrRes.fileInvalid);
    }
  }
  ...
  /// Tìm file video thực tế từ cache của CachedVideoControllerService
  ///
  /// Vấn đề: CachedVideoControllerService thường trả về file .bin không thể lưu trực tiếp
  /// Giải pháp: Tạo file video mới từ cache với tên duy nhất dựa trên URL
  ///
  /// Các bước xử lý:
  /// 1. Kiểm tra xem file cache đã có extension video hợp lệ chưa
  /// 2. Tạo file video mới từ cache với tên duy nhất dựa trên URL hash
  /// 3. Không tìm kiếm trong thư mục cache để tránh lưu nhầm video cũ
  /// 4. Kiểm tra magic bytes để xác định loại file video nếu cần
  static Future<File?> findActualVideoFileFromCache(
      File cachedFile, String originalUrl) async {
    try {
      // Lấy extension từ URL gốc
      final fileName = originalUrl.split('/').last;
      final fileExtension =
          fileName.contains('.') ? fileName.split('.') : ['', ''];
      final extension =
          fileExtension.length > 1 ? fileExtension.last.toLowerCase() : '';

      // Nếu file cache đã có extension video hợp lệ, trả về luôn
      if (['mp4', 'mov', 'avi', 'mkv', 'wmv', 'flv', 'webm', '3gp']
              .contains(extension) &&
          cachedFile.path.endsWith('.$extension')) {
        return cachedFile;
      }

      // Tạo file video mới từ cache với tên duy nhất dựa trên URL
      final cacheDir = cachedFile.parent;
      final urlHash = _generateUrlHash(originalUrl);

      // Không tìm kiếm trong thư mục cache để tránh lưu nhầm video cũ
      // Thay vào đó, tạo file video mới từ cache hiện tại
      ...
      if (extension.isNotEmpty &&
        ...
        try {
          final newFileName =
              '${urlHash}_${DateTime.now().millisecondsSinceEpoch}.$extension';
          final newVideoPath = '${cacheDir.path}/$newFileName';
          final newVideoFile = File(newVideoPath);

          // Copy nội dung từ file cache sang file video mới
          await cachedFile.copy(newVideoPath);

          if (await newVideoFile.exists()) {
            Logger.print('Created video file from cache: $newVideoPath');
            return newVideoFile;
          }
        } catch (e) {
          Logger.print('Failed to create video file from cache: $e');
        }
      }

      // Cuối cùng, thử kiểm tra xem file cache có phải là video thực tế không (dựa vào nội dung)
      try {
        if (cachedFile.lengthSync() > 1024 * 1024) {
          // File lớn hơn 1MB
          // Kiểm tra magic bytes để xác định loại file
          final bytes = await cachedFile.openRead(0, 12).first;
          if (bytes.length >= 4) {
            // Kiểm tra các magic bytes của video formats
            final magicBytes = bytes
                .take(4)
                .map((b) => b.toRadixString(16).padLeft(2, '0'))
                .join('')
                .toLowerCase();

            if (magicBytes.startsWith('000001') || // MP4
                magicBytes.startsWith('66747970') || // MP4
                magicBytes.startsWith('6d6f6f76') || // MOV
                magicBytes.startsWith('52494646')) {
              // AVI, WAV

              // Tạo file video với extension phù hợp
              final videoExtension =
                  _getVideoExtensionFromMagicBytes(magicBytes);
              final newFileName =
                  '${DateTime.now().millisecondsSinceEpoch}.$videoExtension';
              final newVideoPath = '${cacheDir.path}/$newFileName';
              final newVideoFile = File(newVideoPath);
              ...
              if (await newVideoFile.exists()) {
                Logger.print(
                    'Created video file from cache using magic bytes: $newVideoPath');
                return newVideoFile;
              }
            }
          }
        ...
        Logger.print('Failed to check file content: $e');
      }
    } catch (e) {
      Logger.print('Error finding actual video file from cache: $e');
    }

    return null;
  }

  /// Xác định extension video từ magic bytes
  static String _getVideoExtensionFromMagicBytes(String magicBytes) {
    if (magicBytes.startsWith('000001') || magicBytes.startsWith('66747970')) {
      return 'mp4';
    } else if (magicBytes.startsWith('6d6f6f76')) {
      return 'mov';
  ...
  /// Tạo hash duy nhất từ URL để tránh trùng lặp
  static String _generateUrlHash(String url) {
    final uri = Uri.parse(url);
    final path = uri.path;
    final query = uri.query;
    final combined = '$path$query';
  ...
  static Future<bool> checkingBiometric(LocalAuthentication auth) =>
      auth.authenticate(
        localizedReason: StrRes.scanFingerprintForAuth,
  ...
  static bool isValidInviteCode(String text) {
    return RegExp(r'^\d{6}$|^[A-Za-z]{2}\d{4,6}$').hasMatch(text.trim());
  }
}
