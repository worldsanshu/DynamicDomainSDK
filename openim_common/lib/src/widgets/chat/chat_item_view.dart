...
class ChatItemView extends StatefulWidget {
  const ChatItemView({
    ...
    this.userRemarkMap = const {},
    this.quoteMsgSenderNickname,
    ...
    this.shouldShowNicknameCallback,
  });
  ...
  final Map<String, String> userRemarkMap;
  final String? quoteMsgSenderNickname;
  ...
  final Function(Message message)? shouldShowNicknameCallback;
...
class _ChatItemViewState extends State<ChatItemView> {
  ...
  Widget _buildChildView() {
    ...
    } else if (false) { // Removed meeting type check
    ...
    if (_message.isTextType) {
      ...
      String title = _message.mergeElem?.title ?? '';
    ...
    // print sendTime for debug in dev only
  ...
  Widget? get _quoteMsgView {
    ...
    final senderNickname =
        widget.quoteMsgSenderNickname ?? quoteMsg.quoteMessage?.senderNickname;
  ...
  bool _shouldShowLeftNickname() {
    // Nếu không hiển thị nickname bên trái, return false
    if (!widget.showLeftNickname) {
      return false;
    }

    // Nếu có timelineStr, đây là tin nhắn đầu tiên trong nhóm thời gian
    // hoặc tin nhắn đầu tiên từ người gửi mới
    if (widget.timelineStr != null) {
      return true;
    }

    // Sử dụng callback từ bên ngoài nếu có
    if (widget.shouldShowNicknameCallback != null) {
      return widget.shouldShowNicknameCallback!(_message);
    }
